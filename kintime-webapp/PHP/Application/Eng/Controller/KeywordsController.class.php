      ...
      // cookie('id',37219238);
       ...
       if(isset($address)){
           if(isset($keyword)){       //如果存在地址并且存在关键词
               $row=$s->query("(select s.id,s.stypeid,s.headpic,s.name,s.sming,s.price,s.ping,s.xl from eng_shop as s,food as f where (f.cainame like '%{$keyword}%' or s.name like '%{$keyword}%') and s.id=f.sid and s.address like '%{$address}%' and s.state=1 and  f.state=1 limit {$num},6) union (select id,stypeid,headpic,name,sming,ping,gogo,xl from eng_m_shop where name LIKE '%{$keyword}%' and address LIKE '%{$address}%' and state=1 limit {$num},6) union (select id,stypeid,headpic,name,sming,price,ping,xl from eng_g_shop where name LIKE '%{$keyword}%' and address LIKE '%{$address}%' and state=1 limit {$num},6 )"); 
