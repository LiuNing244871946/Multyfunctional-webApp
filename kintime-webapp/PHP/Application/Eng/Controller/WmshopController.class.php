...
class WmshopController extends Controller { //外卖页面下店铺的一个店铺里面
  ...
  if(isset($id)){
     ...
     //$=explode(",",$hd); //如果用数组就解开它赋值
        $row=$m->query("select name,headpic from eng_m_shop where id={$id}"); //此店名字及头像
        $roww=$f->query("select id,cainame,m_xl,price,headpic from eng_food where sid={$id}"); //此店下所有菜
        $rowww=$t->query("select id,name from eng_ftype where sid={$id}"); //此店里面的左侧菜分类
