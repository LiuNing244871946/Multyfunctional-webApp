...
class TypeController extends Controller {  //首页传过来一个地址
   ...
   $address='杭';
       if(isset($address)){
        ...
        $row=$s->query("select * from eng_shop where address LIKE '%{$address}%' and state=1 ") ; //堂食店铺
        $roww=$m->query("select * from eng_m_shop where address LIKE '%{$address}%' and state=1 ") ; //外卖店铺
        $rowww=$g->query("select * from eng_g_shop where address LIKE '%{$address}%' and state=1 ") ; //购物店铺
