...
class MsddanController extends Controller { //当前用户美食订单
    public function index(){  
       ...
       // echo $num;
       if($num==1){ //待消费
          $row=$m->query("select * from eng_t_ddan where uid={$id} and ztai=1 or ztai=6");
            foreach($row as $k=>$v){
               ...
               $rowww=$m->query("select tcname from eng_tc where id={$v['fid']} ")[0]; //查套餐名
                  if($v['ztai']==1){//判断状态
                    $row[$k]['ztai']='待消费';
                    $row[$k]['ckjm']="查看卷码";
                }elseif($v['ztai']==6){
            ...
            $row=$m->query("select * from eng_t_ddan where uid={$id} and ztai=2");
            foreach($row as $k=>$v){
                //$row[$k]['yxtime']=date("Y-m-d H:i:s",$v['time']+(3600*24*3));//有效期至(3天)3600*24*3
               // $row[$k]['time']=date("Y-m-d H:i:s",$row[$k]['time']);//时间转换
               $roww=$m->query("select name,headpic from eng_shop where id={$v['sid']} ")[0];//查店名和头像
               $rowww=$m->query("select tcname from eng_tc where id={$v['fid']} ")[0]; //查套餐名
               $rowwww=$m->query("select did from eng_pj where did={$v['id']} "); //查评价
                   //dump($rowwww);
              ...
              $row=$m->query("select * from eng_t_ddan where uid={$id} and ztai=3");
            foreach($row as $k=>$v){
               $roww=$m->query("select name,headpic from eng_shop where id={$v['sid']} ")[0];//查店名和头像
               $rowww=$m->query("select tcname from eng_tc where id={$v['fid']} ")[0]; //查套餐名
