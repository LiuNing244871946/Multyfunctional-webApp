    ...
    public function index(){
     ...
     if(isset($zn)){//排序下的智能排序
       ...
       if(isset($hp)){//智能排序下的好评优先   
            $row_h=$s->query("select * from shop order by ping desc and wmstate=1 and state=1 limit 0,6");
               echo json_encode($row_h);
            if(isset($hpnum)){
               $row_b=$s->query("select * from shop order by ping desc and wmstate=1 and state=1 limit $hpnum,6");
               echo json_encode($row_b);
            }
        }
       if(isset($rq)){//智能排序下的人气最高   
           $row_x=$s->query("select * from shop order by xiaoliang desc and wmstate=1 and state=1 limit 0,6");
           echo json_encode($row_x);
           if(isset($hpnum)){
            $row_c=$s->query("select * from shop order by xiaoliang desc and wmstate=1 and state=1 limit $hpnum,6");
