...
$type=1;*/
        ...
        $pl = $q_model->query("select p.id,p.dj,p.pics,p.ptime,p.nrong,u.headpic,u.username from pj p,users u,ddan d where p.did = d.id and d.sid = {$id} and p.uid = u.id and d.ztai = 4 AND p.type=1 {$tu} {$xin} limit {$num},6");
...
$id=3;*/
        ...
        $pl = $q_model->query("(select p.id,p.dj,p.pics,p.ptime,p.nrong,u.headpic,u.username from pj p,users u,dz_ddan d where p.did = d.id and d.sid = {$id} and p.uid = u.id and d.ztai = 2 AND p.type = 3 {$tu} {$xin} limit {$num},6) union (select p.id,p.dj,p.pics,p.ptime,p.nrong,u.headpic,u.username from pj p,users u,t_ddan d where p.did = d.id and d.sid = {$id} and p.uid = u.id and d.ztai = 2 AND p.type = 2 {$tu} {$xin} limit {$num},6)");
...
$mai=3;*/
        ...
        elseif($type==3) $tu = "AND p.pics = 2 ORDER BY p.ptime desc";
        elseif($type==4) $xin = "ORDER BY p.dj asc";   
        if($mai==1) $pl = $q_model->query("select p.id,p.dj,p.pics,p.ptime,p.nrong,u.headpic,u.username from ddshop dd,pj p,users u where dd.ddid = p.did and p.uid = u.id and dd.sid = {$id} and p.type = 1 {$tu} {$xin} limit {$num},6"); //外卖
        elseif($mai==2) $pl = $q_model->query("select p.id,p.dj,p.pics,p.ptime,p.nrong,u.headpic,u.username from t_ddan dd,pj p,users u where dd.id = p.did and p.uid = u.id and dd.fid = {$id} and p.type = 2 {$tu} {$xin} limit {$num},6");  //套餐
        elseif($mai==3) $pl = $q_model->query("select p.id,p.dj,p.pics,p.ptime,p.nrong,u.headpic,u.username from dzddshop dzdd,pj p,users u where dzdd.dzddid = p.did and p.uid = u.id and dzdd.sid = {$id} and p.type = 3 {$tu} {$xin} limit {$num},6");  //电子订单
