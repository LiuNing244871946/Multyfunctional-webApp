...
class FoodsController extends CommonController {
    ...
    public function index(){
        ...
        $q_dian = D('pipi_shop');
        $q_tc = D('pipi_tc');
        $q_shop = D('pipi_m_shop');
      	$q_shoppic = D('pipi_shoppic');
    ...
    public function tcan(){
        ...
        $row = $q_model->query("select t.*,s.name from pipi_tc t,pipi_shop s where s.id=t.sid and t.id={$tcan} and tftype=0")[0];
        $sid = $row['sid'];
        $aid = $row['id'];
    ...
    public function mai(){
      ...
      $m_shop = D('pipi_m_shop');
    	$q_food = D('pipi_food');
    	$q_hd = D('pipi_hd');
      $q_ftype = D('pipi_ftype');
    ...
    public function xxi(){
      $id = $_COOKIE['C057DF743DCFDA2C'];
      $m_shoppic = D('pipi_m_shoppic');
      $m_wspic = D('pipi_wspic');
      echo time();
...
$type=1;*/
        ...
        $pl = $q_model->query("select p.id,p.dj,p.pics,p.ptime,p.nrong,u.headpic,u.username from pipi_pj p,pipi_users u,pipi_ddan d where p.did = d.id and d.sid = {$id} and p.uid = u.id and d.ztai = 4 AND p.type=1 {$tu} {$xin} limit {$num},6");
...
$id=3;*/
        ...
        $pl = $q_model->query("(select p.id,p.dj,p.pics,p.ptime,p.nrong,u.headpic,u.username from pipi_pj p,pipi_users u,pipi_dz_ddan d where p.did = d.id and d.sid = {$id} and p.uid = u.id and d.ztai = 2 AND p.type = 3 {$tu} {$xin} limit {$num},6) union (select p.id,p.dj,p.pics,p.ptime,p.nrong,u.headpic,u.username from pipi_pj ppipi_,users u,pipi_t_ddan d where p.did = d.id and d.sid = {$id} and p.uid = u.id and d.ztai = 2 AND p.type = 2 {$tu} {$xin} limit {$num},6)");
    ...
    public function details(){
        ...
        $roww = $model->query("select s.num,s.fid from pipi_shopcar c,pipi_sgoods s where c.uid={$uid} AND c.sid={$sid} AND c.id=s.carid");
        $row['num'] = 0;
        $row['count'] = 0;
    ...
    public function dz_details(){
        ...
        $roww = $model->query("select s.num,s.fid from pipi_dz_shopcar c,pipi_dz_sgoods s where c.uid={$uid} AND c.sid={$sid} AND c.id=s.carid");
        $row['num'] = 0;
        $row['count'] = 0;
        foreach ($roww as $key => $value) {
          $row['count']+=$value['num'];
...
$mai=3;*/
        ...
        elseif($type==3) $tu = "AND p.pics = 2 ORDER BY p.ptime desc";
        elseif($type==4) $xin = "ORDER BY p.dj asc";   
        if($mai==1) $pl = $q_model->query("select p.id,p.dj,p.pics,p.ptime,p.nrong,u.headpic,u.username from pipi_pipi_pipi_ddshop dd,pipi_pipi_pipi_pj p,pipi_pipi_pipi_users u where dd.ddid = p.did and p.uid = u.id and dd.sid = {$id} and p.type = 1 {$tu} {$xin} limit {$num},6"); //外卖
        elseif($mai==2) $pl = $q_model->query("select p.id,p.dj,p.pics,p.ptime,p.nrong,u.headpic,u.username from pipi_pipi_t_ddan dd,pipi_pipi_pj p,pipi_pipi_users u where dd.id = p.did and p.uid = u.id and dd.fid = {$id} and p.type = 2 {$tu} {$xin} limit {$num},6");  //套餐
        elseif($mai==3) $pl = $q_model->query("select p.id,p.dj,p.pics,p.ptime,p.nrong,u.headpic,u.username from pipi_dzddshop dzdd,pipi_pj p,pipi_users u where dzdd.dzddid = p.did and p.uid = u.id and dzdd.sid = {$id} and p.type = 3 {$tu} {$xin} limit {$num},6");  //电子订单
    ...
    public function kandd(){
        ...
        $id = $arr->id;
        $q_model = D();
        $row = $q_model->query("select t.tcname,t.oldprice,t.tcprice,s.name from pipi_shop s,pipi_tc t where t.id={$id} and s.id=t.sid")[0];
        echo json_encode($row);
    }
    //堂食生成订单
    ...
    public function fuq(){
      ...
      $rrow = $model->query("select s.headpic,d.time,t.tcprice from pipi_ddan d,pipi_ddshop dd,pipi_tc t,pipi_shop s where d.id={$id} AND d.sid=s.id AND dd.ddid=d.id AND dd.sid=t.id")[0];
  ...
  public function kgwc(){
    ...
    $q_model = D();
    $row = $q_model->query("select g.num,f.cainame,g.fid,f.zhekou,g.gge from pipi_shopcar c,pipi_sgoods g,pipi_food f where c.uid={$user_id} AND c.sid={$shop_id} AND c.id=g.carid AND g.fid=f.id");
  ...
  public function dgwc(){
    ...
    $row['food'] = $q_model->query("select g.num,f.cainame,f.zhekou,g.gge from pipi_shopcar c,pipi_sgoods g,pipi_food f where c.uid={$user_id} AND c.sid={$shop_id} AND c.id=g.carid AND g.fid=f.id");
  ...
  public function ggwc(){
    ...
    $q_shopcar = D('pipi_shopcar');
    $q_shopcar1 = D('shopcar');
    $q_shopcar2 = D('eng_shopcar');
    $q_sgoods = D('pipi_sgoods');
    $q_sgoods1 = D('sgoods');
...
$shop_id=1;*/
    ...
    $row= $q_shopcar->where("uid={$user_id} AND sid={$shop_id}")->find();
    $carid = $row['id'];
    $g_row = $q_model->query("select s.*,f.cainame,f.zhekou from pipi_sgoods s,pipi_food f where f.id=s.fid AND s.carid={$carid}");
...
/*    $cai = 2;
  ...
  public function dz_ggwc(){
    ...
    $q_shopcar = D('pipi_dz_shopcar');
    $q_shopcar1 = D('dz_shopcar');
    $q_shopcar2 = D('eng_dz_shopcar');
    $q_sgoods = D('pipi_dz_sgoods');
  ...
  public function dz_kgwc(){
    ...
    $q_model = D();
    $row = $q_model->query("select g.num,dz.cainame,g.fid,dz.price,g.gge from pipi_dz_shopcar c,pipi_dz_sgoods g,pipi_dzcd dz where c.uid={$user_id} AND c.sid={$shop_id} AND c.zhuo={$zhuo_id} AND c.id=g.carid AND g.fid=dz.id");
  ...
  public function dz_dgwc(){
    ...
    $row['food'] = $q_model->query("select g.num,dz.cainame,g.fid,dz.price,g.gge from pipi_dz_shopcar c,pipi_dz_sgoods g,pipi_dzcd dz where c.uid={$user_id} AND c.sid={$shop_id} AND c.zhuo={$zhuo_id} AND c.id=g.carid AND g.fid=dz.id");
