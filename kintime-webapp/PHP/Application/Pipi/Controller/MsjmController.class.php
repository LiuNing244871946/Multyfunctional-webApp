...
class MsjmController extends Controller { //查看卷码(美食订单详情)
    public function index(){  
      ...
      // $id=27;
        ...
        $row=$m->query("select * from pipi_t_ddan where uid={$id} and ztai=1 ");
          foreach($row as $k=>$v){
               $row[$k]['time']=date("Y-m-d H:i:s",$row[$k]['time']);//时间转换
               $roww=$m->query("select name,headpic,xiaddress,phone from pipi_shop where id={$v['sid']} ")[0];//查店名,头像,地址,电话
               $rowww=$m->query("select * from pipi_tc,tc_x where tc.id={$v['fid']} and  tc.id=tc_x.tcid")[0]; //查套餐名
