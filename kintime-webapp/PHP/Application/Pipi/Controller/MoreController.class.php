...
class MoreController extends Controller {  //更多商家(美食/堂食)
    public function index(){ 
       ...
       $stypeid=$arr->stypeid;
        ...
        switch($stypeid){
               case 1://美食类
                  $row=$s->query("select id,stypeid,headpic,name,sming,price,ping,xl from pipi_shop limit 0,6 ") ; 
                   foreach ($row as $k => $v) {
                     if($v['id'] == $mid){
                       unset($row[$k]);
                     }
                 ...
                 break;
               case 11: //外卖类
                  $row=$s->query("select id,stypeid,headpic,name,sming,ping,gogo,xl from pipi_m_shop limit 0,6 ") ; 
                   foreach ($row as $k => $v) {
                     if($v['id'] == $wid){
                       unset($row[$k]);
                     }
