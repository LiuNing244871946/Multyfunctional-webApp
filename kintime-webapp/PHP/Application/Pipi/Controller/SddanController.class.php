...
class SddanController extends Controller { //插入到堂食商家订单表
    ...
    //在这里写二维码跳转
    //在这里写二维码跳转
            $s->add($data); //如果付款成功就把用户订单信息插入到商家订单表中
            $s1->add($data); //如果付款成功就把用户订单信息插入到商家订单表中
            $s2->add($data); //如果付款成功就把用户订单信息插入到商家订单表中
            $m->where($id)->save($datb);  //通过id改变用户订单表的状态为"待使用"
            $m1->where($id)->save($datb);  //通过id改变用户订单表的状态为"待使用"
            $m2->where($id)->save($datb);  //通过id改变用户订单表的状态为"待使用"
        }
       // dump($roww);
      /*
       foreach($row as $k=>$v){
