...
class GouwuController extends CommonController { //---------------这个是购物大类的账号
    public function gouwushow(){ 
    	$_SESSION['uid_id'];
    	 //搜索条件
    	...
    	$row = $m->order('id desc')->where("(type=6 or type=7 or type=8)".$map)->limit($Page->firstRow.','.$Page->listRows)->select();
      // dump($row);
      foreach ($row as $k => $v) {
            if($v['state']== 1){
                $row[$k]['state']='正常';
...
if(IS_POST){
    ...
    if(!empty($_POST['pro'])){
       ...
       $res=$m->add($_POST);
 if($res){
       echo "<script>alert('添加成功!');location.href='".U('gouwushow')."'</script>";
   }else{
    ...
    $this->assign('name',$name);
     ...
     $s=D('upasstype');
     $res =$s->where("id=5")->select(); //!-------这里需要改动,因为指定了购物类id为5!!!
     // dump($res); 
     $this->assign('res',$res); 
     $this->display();
    }
